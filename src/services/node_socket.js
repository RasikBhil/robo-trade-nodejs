...
import { ACTION, CONSTANTS, EXCHANGES, MODE } from "../shared/angelOne.js";
let triggers = {
  connect: [],
  tick: [],
};
...
let WebSocketV2 = function (params) {
  try {
    ...
    const url = `${CONSTANTS?.websocketURL}?clientCode=${clientcode}&feedToken=${feedtype}&apiKey=${apikey}`;
      ...
      try {
        return new Promise((resolve, reject) => {
          ...
          ws.onopen = function onOpen(evt) {
            console.log("sub list", subscribeData);
    ...
    this.fetchData = function (json_req) {
      ...
      console.log("ws status", ws, ws?.readyState === open);
      if (ws?.readyState === open) {
...
function setResponse(buf, result) {
  console.log("get response", buf);
  const subscription_mode = new Parser().uint8("subscription_mode");
  console.log("SUB code", subscription_mode.parse(buf)?.subscription_mode);
