    ...
    fetch('https://margincalculator.angelbroking.com/OpenAPI_File/files/OpenAPIScripMaster.json')
        ...
        .then(async response => {
            try {
                ...
                console.log(error);
                console.log('====================================');
                addSchedularStatus("error while adding instrument data , error:-", JSON.stringify(error))
            }
        })
}
...
const addSchedularStatus = async (status) => {
    await schedularJobsDB.collection.insertOne({ "timeStamp": moment(new Date()).format('llll'), "status": status }).then(() => {
...
const deleteSchedularStatus = async () => {
    ...
    await schedularJobsDB.collection.drop().then(() => {
...
export { addInstruments, deleteInstruments, fetchInstrumentData, addSchedularStatus, deleteSchedularStatus }